...
const mapGeminiRuntimeErrorCode = (
  ...
  if (code === 'GEMINI_API_KEY_MISSING') {
    return 'Gemini runtime key pool is empty. Configure GEMINI_API_KEYS or GEMINI_API_KEY.';
  }
...
const parseRuntimeErrorDetail = (payload: any, status: number, statusText: string): string => {
  ...
  const errorCode = String(detailObj?.errorCode || payload?.errorCode || '').trim();
  const summary = String(detailObj?.summary || payload?.summary || detailObj?.error || payload?.error || '').trim();
