...
export const MainApp: React.FC<MainAppProps> = ({ setScreen }) => {
  ...
  const [uiDensity, setUiDensity] = useState<UiDensity>(() => {
    const saved = localStorage.getItem('vf_ui_density');
    return saved === 'comfortable' ? 'comfortable' : 'compact';
  });
  const [uiFontScale, setUiFontScale] = useState<number>(() => {
    const saved = parseFloat(localStorage.getItem('vf_ui_font_scale') || '1');
  ...
  const [studioEditorMode, setStudioEditorMode] = useState<StudioEditorMode>(() => {
      const saved = localStorage.getItem('vf_studio_editor_mode');
  ...
  const fetchRuntimeJson = async (url: string, timeoutMs: number = 3000): Promise<any> => {
  ...
  const ensureEngineOnline = async (
      ...
      if (!runtimeUrl) {
          throw new Error(
              engine === 'GEM'
                  ? 'Set Plus Runtime URL in Settings (for example http://127.0.0.1:7810).'
                  : 'Set Basic Runtime URL in Settings (for example http://127.0.0.1:7820).'
          );
      }
      ...
      try {
          ...
          try {
              ...
              if (
                  ...
                  throw new Error(`Media backend is unreachable at ${mediaBackendUrl}. Start services with "npm run services:bootstrap" or update Media Backend URL in Settings.`);
              }
  ...
  // --- Effects ---
  useEffect(() => { localStorage.setItem('vf_ui_theme', uiTheme); }, [uiTheme]);
  useEffect(() => { localStorage.setItem('vf_ui_density', uiDensity); }, [uiDensity]);
  useEffect(() => { localStorage.setItem('vf_ui_font_scale', String(uiFontScale)); }, [uiFontScale]);
  useEffect(() => { localStorage.setItem('vf_studio_editor_mode', studioEditorMode); }, [studioEditorMode]);
  useEffect(() => {
      setSettings((prev) => {
          const next = {
              ...prev,
    ...
    if (isWalletBlocked) {
      ...
      if (wantsTonePreservation && !settings.rvcModel) {
          patchDubbingUiState({
              ...
              error: 'Enable tone preservation requires selecting an RVC model in Settings.',
          });
          return showToast('Enable tone preservation requires selecting an RVC model in Settings.', 'info');
      }
      try {
          if (videoFile) {
  ...
  const blobUrlToFile = async (url: string, filename: string, type: string): Promise<File> => {
  ...
  const openAuthScreen = (mode: 'login' | 'signup') => {
      try {
          localStorage.setItem('vf_auth_intent', mode);
      } catch {
          // no-op
      }
  ...
  const handleStartServices = () => {
      showToast('Run \"npm run bootstrp\" in a terminal to start backend services.', 'info');
      void refreshBackendHealth(true);
      void refreshTtsRuntimeStatus();
  };
    ...
    <aside className={`fixed inset-y-0 left-0 z-[56] md:z-40 w-64 bg-white border-r border-gray-100 shadow-2xl md:shadow-none transform transition-transform duration-300 md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-1">
            {[
                ...
                { id: Tab.NOVEL, icon: <BookOpen size={18} />, label: 'Novel Workspace' },
      ...
      >
          ...
          >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-gray-50/50">
                  ...
                  {/* Media Backend */}
                  <section>
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 block">Media Backend</label>
                      <div className="space-y-3 animate-in fade-in bg-white p-3 rounded-xl border border-gray-100">
                          <div>
                              <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Server size={10}/> Backend URL</label>
                              <input
                                  type="text"
                                  value={settings.mediaBackendUrl || ''}
                                  onChange={(e) => setSettings(s => ({ ...s, mediaBackendUrl: e.target.value }))}
                                  placeholder="http://127.0.0.1:7800"
                                  className="w-full p-2.5 text-xs border border-gray-200 rounded-lg outline-none focus:border-indigo-500 font-mono bg-gray-50 focus:bg-white transition-colors"
                              />
                              <p className="text-[10px] text-gray-400 mt-1">Used for real RVC conversion and FFmpeg video dubbing.</p>
                          </div>
                          <div>
                              <div className="flex items-center justify-between gap-2 mb-1">
                                  <label className="text-[10px] font-bold text-gray-500 uppercase flex items-center gap-1"><Key size={10}/> Backend API Key</label>
                                  <button
                                      type="button"
                                      onClick={async () => {
                                          try {
                                              ...
                                              setSettings((s) => ({ ...s, backendApiKey: nextKey }));
                                              showToast('Backend API key pasted.', 'success');
                                          } catch {
                                              showToast('Clipboard access failed. Paste manually.', 'error');
                                          }
                                      }}
                              ...
                              <input
                                  ...
                                  onChange={(e) => setSettings(s => ({ ...s, backendApiKey: e.target.value }))}
                                  placeholder="Optional: bearer token for OpenAI/F5 compatible backends"
                                  className="w-full p-2.5 text-xs border border-gray-200 rounded-lg outline-none focus:border-indigo-500 font-mono bg-gray-50 focus:bg-white transition-colors"
                              />
                          </div>
                          <div>
                              <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Sparkles size={10}/> Plus Runtime URL</label>
                              <input
                                  type="text"
                                  value={settings.geminiTtsServiceUrl || ''}
                                  onChange={(e) => setSettings(s => ({ ...s, geminiTtsServiceUrl: e.target.value }))}
                                  placeholder="http://127.0.0.1:7810"
                                  className="w-full p-2.5 text-xs border border-gray-200 rounded-lg outline-none focus:border-indigo-500 font-mono bg-gray-50 focus:bg-white transition-colors"
                              />
                          </div>
                          <div>
                              <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Cpu size={10}/> Basic Runtime URL</label>
                              <input
                                  type="text"
                                  value={settings.kokoroTtsServiceUrl || ''}
                                  onChange={(e) => setSettings(s => ({ ...s, kokoroTtsServiceUrl: e.target.value }))}
                                  placeholder="http://127.0.0.1:7820"
                                  className="w-full p-2.5 text-xs border border-gray-200 rounded-lg outline-none focus:border-indigo-500 font-mono bg-gray-50 focus:bg-white transition-colors"
                              />
                          </div>
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                               <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-2 cursor-pointer">
                                   <input
                                       type="checkbox"
                                       checked={settings.useModelSourceSeparation !== false}
                                       onChange={(e) => setSettings((s) => ({ ...s, useModelSourceSeparation: e.target.checked }))}
                                       className="mt-0.5 accent-indigo-600"
                                   />
                                   <span className="text-[11px] leading-4 text-gray-700">
                                       <strong className="block text-gray-800">Model Stem Separation</strong>
                                       Use Demucs backend model for speech/background split.
                                   </span>
                               </label>
                               <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-2 cursor-pointer">
                                   <input
                                       type="checkbox"
                                       checked={Boolean(settings.preserveDubVoiceTone)}
                                       onChange={(e) => setSettings((s) => ({ ...s, preserveDubVoiceTone: e.target.checked }))}
                                       className="mt-0.5 accent-indigo-600"
                                   />
                                   <span className="text-[11px] leading-4 text-gray-700">
                                       <strong className="block text-gray-800">Preserve Voice Tone</strong>
                                       Apply RVC pass on dubbed speech (requires selected RVC model).
                                   </span>
                               </label>
                           </div>
                           <div className="flex items-center gap-2">
                               <button
                                  ...
                                  className="px-3 py-2 text-[11px] font-bold rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-60 flex items-center gap-1.5"
                              >
                                  {isCheckingBackend ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
                                  Check Health
                              </button>
                              <button
                                  onClick={() => { void refreshRvcModels(); }}
                                  disabled={isLoadingRvcModels}
                                  className="px-3 py-2 text-[11px] font-bold rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 disabled:opacity-60 flex items-center gap-1.5"
                              >
                                  {isLoadingRvcModels ? <Loader2 size={12} className="animate-spin" /> : <Music size={12} />}
                                  Sync RVC Models
                              </button>
                          </div>
                          {backendHealth && (
                              <div
                                className={`text-[10px] rounded-lg px-2 py-1.5 border ${
                                  backendHealth.severity === 'ok'
                                    ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                                    : backendHealth.severity === 'warn'
                                      ? 'bg-amber-50 text-amber-700 border-amber-200'
                                      : 'bg-red-50 text-red-700 border-red-200'
                                }`}
                              >
                                  {backendHealth.summary}
                              </div>
                          )}
                      </div>
                  </section>
                             ...
                             <div className="space-y-5">
                                 ...
                                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                     <div className="space-y-2">
                                         <label className="text-xs font-bold text-gray-500 uppercase">Backend</label>
                                         <div className="p-3 rounded-xl border border-gray-200 bg-gray-50 text-xs font-mono">
                                             {mediaBackendUrl}
                                         </div>
                                         <div className="flex gap-2">
                                             <button
                                                onClick={() => { void refreshBackendHealth(); }}
                                                disabled={isCheckingBackend}
                                                className="px-3 py-2 text-xs font-bold rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50"
                                             >
                                                {isCheckingBackend ? 'Checking...' : 'Check'}
                                             </button>
                                             <button
                                                onClick={() => { void refreshRvcModels(); }}
                                                disabled={isLoadingRvcModels}
                                                className="px-3 py-2 text-xs font-bold rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 disabled:opacity-50"
                                             >
                                                {isLoadingRvcModels ? 'Loading...' : 'Refresh Models'}
                                             </button>
                                         </div>
                                          {backendHealth && (
                                            <div
