...
const DEFAULT_SOURCE_MANIFEST = path.join(ROOT, 'config', 'voice_transfer_model_sources.json');
const DEFAULT_OUTPUT_MANIFEST = path.join(ROOT, 'data', 'voice-transfer-model-download-manifest.json');
const DEFAULT_MODELS_ROOT = path.resolve(
  ROOT,
  String(process.env.VF_VOICE_TRANSFER_MODELS_DIR || 'models/voice-transfer').trim() || 'models/voice-transfer'
);

function parseArgs(argv) {
  const args = {
    force: false,
    sourceManifest: process.env.VF_VOICE_TRANSFER_MODEL_SOURCE_MANIFEST || DEFAULT_SOURCE_MANIFEST,
    outputManifest: process.env.VF_VOICE_TRANSFER_MODEL_DOWNLOAD_MANIFEST || DEFAULT_OUTPUT_MANIFEST,
    modelsRoot: process.env.VF_VOICE_TRANSFER_MODELS_DIR || DEFAULT_MODELS_ROOT,
  };
