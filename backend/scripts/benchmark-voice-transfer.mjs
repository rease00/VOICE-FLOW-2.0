...
const MODEL_NAME = process.env.VF_BENCH_MODEL || "voice_transfer_hq_cpu";
const OUT_FILE = path.join(ROOT, "artifacts", "llvc_benchmark_report.json");

const PRESETS = ["voice_transfer_hq_cpu", "tts_realtime"];
...
async function runOne(preset, inputBytes, inputName) {
  ...
  const response = await fetch(`${BACKEND_URL}/voice-transfer/convert`, { method: "POST", body: form });
