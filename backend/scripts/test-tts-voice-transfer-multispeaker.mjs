...
const { headers: AUDIT_AUTH_HEADERS, auth: AUDIT_AUTH } = buildAuditHeaders(
  ...
  { scriptName: 'test:tts:voice-transfer:multispeaker', defaultDevUid: TEST_UID }
);
...
async function main() {
  ...
  try {
    ...
    try {
      llvcModelsPayload = await fetchJson(`${BACKEND_URL}/voice-transfer/models`);
    } catch (error) {
      report.warnings.push(
        `/voice-transfer/models unavailable; direct /voice-transfer/convert gateway check may be skipped (${error instanceof Error ? error.message : String(error)})`
      );
    }
    ...
    if (firstAudioForRvc) {
      ...
      if (fallbackModel) {
        ...
        const response = await fetchWithTimeout(
          `${BACKEND_URL}/voice-transfer/convert`,
          {
            method: 'POST',
            headers: AUDIT_AUTH_HEADERS,
            body: form,
        ...
        if (!response.ok) {
          ...
          report.failures.push(`/voice-transfer/convert failed (${response.status}) ${text.slice(0, 260)}`);
          ...
          ensureAudioBytes(bytes, '/voice-transfer/convert');
          report.directRvcGateway = {
            ok: true,
            statusCode: response.status,
            modelName: fallbackModel,
        ...
        report.warnings.push('Skipped /voice-transfer/convert direct check: no models returned from /voice-transfer/models');
      }
    } else {
      report.warnings.push('Skipped /voice-transfer/convert direct check: no successful synthesis output available.');
    }
