...
def test_merge_env_files_uses_env_local_fallback(tmp_path: Path) -> None:
    ...
    repo_env_local.write_text(
        'FIREBASE_SEED_ADMIN_PASSWORD=local\nNEXT_PUBLIC_ADMIN_LOGIN_EMAIL=admin1@voiceflow.local\n',
        encoding='utf-8',
    )
    backend_env = tmp_path / 'backend.env'
    backend_env.write_text('NEXT_PUBLIC_ADMIN_EMAIL_ALLOWLIST=admin2@voiceflow.local\n', encoding='utf-8')
    ...
    assert merged['NEXT_PUBLIC_ADMIN_EMAIL_ALLOWLIST'] == 'admin2@voiceflow.local'
    assert merged['NEXT_PUBLIC_ADMIN_LOGIN_EMAIL'] == 'admin1@voiceflow.local'
...
def test_load_rows_from_allowlists_ignores_public_admin_envs_by_default() -> None:
    env_values = {
        'NEXT_PUBLIC_ADMIN_EMAIL_ALLOWLIST': 'admin1@voiceflow.local',
        'NEXT_PUBLIC_ADMIN_UID_ALLOWLIST': 'uid-1,uid-2',
    }
...
def test_load_rows_from_allowlists_allows_public_admin_envs_when_opted_in() -> None:
    ...
    assert {row.email for row in rows if row.email} == {'admin1@voiceflow.local', 'admin-uid1@voiceflow.local', 'admin-uid2@voiceflow.local'}
...
def test_load_rows_from_allowlists_reads_server_only_email_allowlist() -> None:
    env_values = {
        'VF_ADMIN_APPROVER_EMAILS': 'admin2@voiceflow.local',
    }
    ...
    assert rows[0].email == 'admin2@voiceflow.local'
