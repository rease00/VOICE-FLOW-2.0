...
import type { StandardPodcastJobRequest } from '../model/podcast';

const withBaseUrl = (baseUrl?: string): { baseUrl?: string } => (baseUrl ? { baseUrl } : {});

export const createStandardPodcastJob = async (
  payload: StandardPodcastJobRequest,
...
export const getStandardPodcastJob = async (
  jobId: string,
  options?: {
  ...
  const path = searchParams.toString()
    ? `/podcast/standard/jobs/${safeJobId}?${searchParams.toString()}`
    : `/podcast/standard/jobs/${safeJobId}`;
  return requestJson<TtsJobStatusResponse>(
    path,
    undefined,
    { ...withBaseUrl(options?.baseUrl), requireAuth: true }
...
export const cancelStandardPodcastJob = async (
  jobId: string,
  options?: { baseUrl?: string }
): Promise<TtsJobStatusResponse> => {
  const safeJobId = encodeURIComponent(String(jobId || '').trim());
...
export const fetchStandardPodcastChunkAudio = async (
  jobId: string,
  chunkIndex: number,
  baseUrl?: string
): Promise<ArrayBuffer> => {
  ...
  const blob = await requestBlob(
    `/podcast/standard/jobs/${safeJobId}/chunks/${safeChunkIndex}`,
    undefined,
...
export const fetchStandardPodcastAudio = async (
  jobId: string,
  options?: { baseUrl?: string }
): Promise<{ audioBytes: ArrayBuffer; responseHeaders: Record<string, string> }> => {
  const safeJobId = encodeURIComponent(String(jobId || '').trim());
  const blob = await requestBlob(
    `/podcast/standard/jobs/${safeJobId}/audio`,
    undefined,
    { ...withBaseUrl(options?.baseUrl), requireAuth: true }
  );
  return {
