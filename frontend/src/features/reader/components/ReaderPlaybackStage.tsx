    ...
    <div className="vf-reader-player" data-testid="reader-playback-stage">
      <section className="vf-reader-player__hero">
        <div className="vf-reader-player__headline">
          ...
          <h2>{session.title}</h2>
          <p>{session.summary || (session.contentKind === 'comic' ? 'Panel-aware reading with audio-synced pacing.' : 'Continuous narrated reading with live text highlight and low-latency prefetch.')}</p>
      ...
      <section className="vf-reader-player__focus">
        <div className="vf-reader-player__focus-head">
          <strong>{activeItem?.title || 'Preparing next unit'}</strong>
          <div className="vf-reader-player__focus-language">
            <Languages size={14} />
            <span>{session.ttsLanguageMode === 'target' ? `Narrating in ${targetLanguageLabel}` : 'Narrating from the source script'}</span>
          </div>
        </div>
        <p>{activeText}</p>
      </section>
      ...
      {session.contentKind === 'book' ? (
        ...
        <section className="vf-reader-player__comic-stage" onWheel={pauseAutoSwipe} onTouchStart={pauseAutoSwipe}>
