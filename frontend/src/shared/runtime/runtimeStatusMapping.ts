export {
  ENGINE_COMPACT_LABELS,
  ENGINE_DISPLAY_NAMES,
  ENGINE_RUNTIME_SUBLABELS,
  sanitizeTtsEngineText,
} from '../../../services/engineDisplay';

// ── Inlined from the deleted services/runtimeStatusMapping.ts ──
...
const ENGINE_RUNTIME_STATE_SET: ReadonlySet<EngineRuntimeUiState> = new Set([
  'checking', 'starting', 'warming', 'online', 'offline', 'not_configured', 'standby',
]);
...
const RUNTIME_LANE_LABELS: Record<string, string> = {
  vector: 'VECTOR', vec: 'VECTOR', prime: 'PRIME', PRIME: 'PRIME', gemini: 'PRIME',
};

const GCP_REGION_LABELS: Record<string, string> = {
  asia: 'Asia', 'asia-south1': 'Mumbai', 'asia-south2': 'Delhi',
  'asia-southeast1': 'Singapore', 'asia-east1': 'Taipei', 'asia-northeast1': 'Tokyo',
  eu: 'EU', europe: 'Europe', 'europe-west1': 'Belgium', 'europe-west4': 'Netherlands',
  'europe-west2': 'London', 'europe-west3': 'Frankfurt', 'europe-central2': 'Poland',
  'europe-north1': 'Finland', 'europe-southwest1': 'Madrid',
  us: 'US', 'us-central1': 'Iowa', 'us-east1': 'South Carolina', 'us-east4': 'Northern Virginia',
  'us-east5': 'Columbus', 'us-south1': 'Dallas', 'us-west1': 'Oregon', 'us-west4': 'Las Vegas',
  'northamerica-northeast1': 'Montreal', global: 'Global',
};
...
const normalizeRuntimeMetadataLabel = (field: keyof EngineRuntimeMetadata, value: unknown): string => {
  ...
  if (!token) return '';
  if (field === 'provider') return RUNTIME_PROVIDER_LABELS[token.toLowerCase()] || token;
  if (field === 'lane') return RUNTIME_LANE_LABELS[token.toLowerCase()] || token;
  return token;
};
...
const formatRuntimeLocationLabel = (value: unknown): string => {
  ...
  if (!token) return '';
  return GCP_REGION_LABELS[token.toLowerCase()] || token;
};
...
export const formatRuntimeServerLabel = (status: Partial<EngineRuntimeMetadata> | null | undefined): string => {
  ...
  if (!token) return '';
  try { return new URL(token).host || token; } catch { return token.replace(/^https?:\/\//i, '').split(/[/?#]/)[0] || token; }
};
...
const buildUiStatus = (
  state: EngineRuntimeUiState, detail: string, metadataSource: Record<string, unknown> | null
...
export const normalizeEngineRuntimeStateToken = (
  rawState: unknown, fallback: EngineRuntimeUiState = 'offline'
...
export const mapGatewayEngineRuntimeToUiStatus = (engineItem: unknown): EngineRuntimeUiStatus => {
  ...
  const candidate = engineItem as Record<string, unknown> & {
    ready?: unknown; capabilities?: Record<string, unknown> | null;
  ...
  if (stateToken === 'not_configured') return buildUiStatus('not_configured', detail, metadataSource);
  if (stateToken === 'warming') return buildUiStatus(standbyHint ? 'standby' : 'starting', detail, metadataSource);
  if (stateToken === 'starting' || (stateToken === 'online' && !runtimeReady)) return buildUiStatus('starting', detail, metadataSource);
  if (stateToken === 'online') return buildUiStatus('online', detail, metadataSource);
  if (stateToken === 'standby' || (stateToken === 'offline' && standbyHint)) return buildUiStatus('standby', detail, metadataSource);
