...
import { verifyFirebaseRequest as verifyAuthedStudioRequest } from '../auth/requestAuth';
import {
...
const buildNativeStudioEngineRuntimeUrl = (request: Request): string => {
  try {
    const url = new URL(request.url);
    url.pathname = '/api/v1/studio/tts/synthesize';
    url.search = '';
    return url.toString();
  } catch {
    return '/api/v1/studio/tts/synthesize';
  }
};
...
const synthesizeStudioAudio = async (
...
const verifyFirebaseRequest = async (request: Request): Promise<void> => {
  const token = sanitizeHeaderBearerToken(request.headers.get('authorization'));
  if (!token) {
...
const synthesizeStudioChunk = async (chunk: string, request: ReaderStudioSynthesizeRequest): Promise<Buffer> => {
...
export const handleStudioSynthesizeRoute = async (request: Request): Promise<Response> => {
  try {
    ...
    return await synthesizeStudioAudio(body);
  } catch (error) {
...
export const handleStudioLongTextRoute = async (request: Request): Promise<Response> => {
  try {
    ...
    for (const chunk of chunks) {
      const response = await synthesizeStudioAudio({ ...body, text: chunk }, 'wav');
      if (!response.ok) {
      ...
      if (!modelHeader) {
        modelHeader = String(response.headers.get('X-TTS-Model') || '').trim();
      }
    }
      ...
      async start(controller) {
        try {
          for (let i = 0; i < chunks.length; i += 1) {
            ...
            try {
              const audio = await synthesizeStudioChunk(chunk, body);
