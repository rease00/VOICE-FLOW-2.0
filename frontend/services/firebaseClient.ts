...
const firebaseConfig = {
  apiKey: readEnvValue(process.env.NEXT_PUBLIC_FIREBASE_API_KEY) || DEFAULT_DEPLOY_FIREBASE_CONFIG.apiKey,
  authDomain: readEnvValue(process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN) || DEFAULT_DEPLOY_FIREBASE_CONFIG.authDomain,
  projectId: readEnvValue(process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID) || DEFAULT_DEPLOY_FIREBASE_CONFIG.projectId,
  ...
  appId: readEnvValue(process.env.NEXT_PUBLIC_FIREBASE_APP_ID) || DEFAULT_DEPLOY_FIREBASE_CONFIG.appId,
};
...
const REQUIRED_FIREBASE_ENV_KEYS = [
  { key: 'apiKey', primary: 'NEXT_PUBLIC_FIREBASE_API_KEY' },
  { key: 'authDomain', primary: 'NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN' },
  { key: 'projectId', primary: 'NEXT_PUBLIC_FIREBASE_PROJECT_ID' },
  { key: 'appId', primary: 'NEXT_PUBLIC_FIREBASE_APP_ID' },
] as const;
...
const adminEmailAllowlist = new Set(
  readEnvCsv(process.env.NEXT_PUBLIC_ADMIN_EMAIL_ALLOWLIST).map((item) => item.toLowerCase())
);

const adminUidAllowlist = new Set(
  readEnvCsv(process.env.NEXT_PUBLIC_ADMIN_UID_ALLOWLIST)
);

if (!hasRequiredFirebaseConfig && process.env.NODE_ENV === 'development') {
  // eslint-disable-next-line no-console
  console.warn('Firebase config is incomplete. Set NEXT_PUBLIC_FIREBASE_* environment variables.');
}
...
export let firebaseApp = getOrCreateApp(hasRequiredFirebaseConfig ? firebaseConfig : FALLBACK_FIREBASE_CONFIG);
...
const buildFirebaseConfigIssue = (): string => {
  if (missingRequiredFirebaseEnv.length > 0) {
    const requiredVars = missingRequiredFirebaseEnv.map((item) => item.primary);
