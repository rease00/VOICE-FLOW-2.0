...
describe('voice clone stem separation request builder', () => {
  it('defaults the compressed upload cap above a common 10 minute mp3 size', async () => {
    const previousNext = process.env.NEXT_PUBLIC_OPENVOICE_MAX_AUDIO_BYTES;
    const previousVite = process.env.VITE_OPENVOICE_MAX_AUDIO_BYTES;
    process.env.NEXT_PUBLIC_OPENVOICE_MAX_AUDIO_BYTES = '';
    process.env.VITE_OPENVOICE_MAX_AUDIO_BYTES = '';

    try {
      const { getOpenVoiceStemExtractionMaxBytes } = await import('../src/features/voice-cloning/stemSeparation');
      expect(getOpenVoiceStemExtractionMaxBytes()).toBe(12 * 1024 * 1024);
    } finally {
      if (typeof previousNext === 'undefined') {
        delete process.env.NEXT_PUBLIC_OPENVOICE_MAX_AUDIO_BYTES;
      } else {
        process.env.NEXT_PUBLIC_OPENVOICE_MAX_AUDIO_BYTES = previousNext;
      }
      if (typeof previousVite === 'undefined') {
        delete process.env.VITE_OPENVOICE_MAX_AUDIO_BYTES;
      } else {
        process.env.VITE_OPENVOICE_MAX_AUDIO_BYTES = previousVite;
      }
    }
  });

  it('sends the original compressed source and preserves trim metadata', async () => {
    const { buildOpenVoiceStemSeparationRequest, isFullDurationTrimRange } = await import(
    ...
    const request = await buildOpenVoiceStemSeparationRequest({
      sourceAudio,
